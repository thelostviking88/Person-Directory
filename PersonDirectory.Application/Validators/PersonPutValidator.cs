...
namespace PersonDirectory.Application.Validators
{
    public class PersonPutValidator : AbstractValidator<PersonPutDto>
    {
        ...
        public PersonPutValidator(IStringLocalizer<SharedValidatorResources> localizer)
        {
            ValidatorOptions.Global.LanguageManager.Culture = System.Threading.Thread.CurrentThread.CurrentCulture;

            RuleFor(x => x.FirstName).
                NotEmpty().WithMessage(x => localizer["NameIsRequired"]).
                Matches(@"^[a-zA-Z]{2,50}$|^[ა-ჰ]{2,50}$")
                .WithMessage(x => localizer["NameIs2-50LatinOrGeorgian"]);

            RuleFor(x => x.LastName)
                .NotEmpty().WithMessage(x => localizer["LastNameIsRequired"]).
                Matches(@"^[a-zA-Z]{2,50}$|^[ა-ჰ]{2,50}$")
                .WithMessage(x => localizer["LastNameIs2-50LatinOrGeorgian"]);
            ...
            RuleFor(x => x.DateOfBirth).Must(birth => birth < DateTime.Now.AddYears(-18))
                .WithMessage(x => localizer["DateOfBirthMin18Years"]);
